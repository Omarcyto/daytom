...
public class ExcelManager {
    ...
    public ExcelManager() throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            this.source = new File(new File(".").getCanonicalPath() + SEPARATOR + "createInvoiceCI.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        this.createCustomsInvoice();

    }

    public void createCustomsInvoice() {
        this.createCustomsInvoiceFromExcel(source);
...
//                ));
    ...
    public void createCustomsInvoiceFromExcel(File source) {
        this.productList = new ArrayList<>();
        this.counter = 1;
        Workbook sourceData = this.getWorkBook(source);
        ...
        sourceData.getSheetAt(0)
                .forEach(row -> {
                    if (cont[0] == 0 || cont[0] == 1) {
                        ...
                        if (!row.getCell(0).getStringCellValue().replace(" ", "").equalsIgnoreCase("")) {
                            String formatCode = row.getCell(0).getStringCellValue().replace(" ", "").toUpperCase();
                            String finalCode = formatCode.startsWith("E.P")
                                    ? formatCode.substring(3, formatCode.length())
                                    : formatCode;

                            productList.add(GenericBuilder.of(Product::new)
                                    .with(Product::setCodOrigin, finalCode)  //ok .
                                    .with(Product::setBoxNumber, row.getCell(1).getStringCellValue())
                                    .with(Product::setQuantityOrigin, row.getCell(2).getStringCellValue())
                                    .build());
        ...
        productList.forEach(product -> System.out.println(product.getCodOrigin()+" "+product.getBoxNumber()));
        System.out.println("Total rows: " + counter);

        StringJoiner sj = new StringJoiner(", ");
      //  String test = "^2S[ACDK]"; //2SC 2SD 2SK
     //   String test = "^BU";
        String test = "^PA[\\d]|^PAL|^MX|^6N"; //74 LS
        Pattern p = Pattern.compile(test);
        Matcher m = p.matcher("BU34");
        ...
        System.out.println(sj.toString() + " Series");

    }

    public void organizePerBox(){
